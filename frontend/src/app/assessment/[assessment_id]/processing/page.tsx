...
import { ArrowRight, CheckCircle2, CircleDot, Loader2, RefreshCcw } from "lucide-react";
...
export default function ProcessingPage() {
  ...
  // Update polling function untuk pakai isMounted.current
  const startPolling = () => {
    const poll = async () => {
      try {
        ...
        const message = err instanceof Error ? err.message : "Gagal memuat progres";
        setError(message);
      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-6">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-blue-900/40 backdrop-blur">
          <div className="flex flex-col gap-2">
            <div>
              <h1 className="text-2xl font-bold sm:text-3xl">Memproses Assessment</h1>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <p className={`mt-2 text-xs font-semibold ${visual.color}`}>
                    {isDone && "✓ Selesai"}
                    {isActive && "⏳ Sedang berjalan"}
                    {!isDone && !isActive && "⏸ Menunggu giliran"}
                  </p>
